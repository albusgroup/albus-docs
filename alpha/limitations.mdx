...
description: What Albus does not do yet, and what arrives in beta.
---

Albus is in alpha. Everything below is deliberate scope, not a defect — read it
before you build on the platform, because each item changes what you should ask
an agent to do.

## Quota: 20 session runs per organization

Every invocation counts, across every session, for the lifetime of the
organization. Once the cap is reached, a run returns `429` with
...
are all beta work, along with a higher run cap. Tell
