...
{/* TODO(10): fold the worked example into Build-an-agent step 3; drop the standalone page. */}
...
| `502` mid-run | Every attempt of the invocation failed; `GET /traces/{invocation_key}` and `albus sessions audit <session>` show the last `tool_call` and the failure |
