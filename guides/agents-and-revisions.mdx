...
{/* TODO(10): fold into Build-an-agent step 5 (each change = new revision; view revisions in CLI and web UI); drop the standalone page. */}

Trace a run back to the exact agent configuration that produced it. You never
create an agent: you name one on a run, and Albus records it.
...
It is also how you read a whole agent's history back rather than one session's:
`GET /traces?agent_name=support-triage&agent_revision=<revision>` lists every
invocation that ran that configuration, whichever session it ran in.
