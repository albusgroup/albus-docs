...
A server you host yourself needs no `headers` entry: when `headers` has no
`Authorization` entry, Albus sends a signed identity token, which your server
verifies against your organization's issuer. See
[Authenticate Albus to your MCP server](/guides/mcp-server-auth).
