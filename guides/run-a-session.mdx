...
`wait_timeout_seconds` bounds the wait server-side, in seconds. If it elapses
first the request fails with **`504`** and a JSON body — the run is still going,
and retrying the same idempotency key re-attaches to it. Pass `0` to wait until
the reply arrives or your client disconnects; that is what the CLI sends for
`--wait` without `--wait-timeout`.

**Don't wait.** Return as soon as the invocation is accepted, then poll. Omit
`wait_timeout_seconds` over HTTP; pass `--no-wait` in the CLI.
