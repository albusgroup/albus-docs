...
header, the value is wrong: store the *complete* header value including the
scheme — `Bearer <token>`, not `<token>`. With `oauth2_client_credentials`, the
token obtained at launch has expired; it is not refreshed. Details in
