...
- `wait_timeout_seconds=<n>` blocks up to `n` seconds until the assistant replies
  and returns the session with its messages. `wait_timeout_seconds=0` waits
  without a bound. Omit the parameter to return as soon as the run is accepted,
  then poll `GET /sessions/{id}` until `session.state` leaves `RUNNING`.
- Exceeding the wait is `504`, and the run is still going.
- The CLI spells the same thing as `--wait`/`--no-wait` and `--wait-timeout`:
  `--wait` (the default) with no `--wait-timeout` sends `0`, and `--no-wait` omits
  the parameter.
...
// @albus-ts/sdk 0.4.0 cannot send wait_timeout_seconds yet, so start the run
// and poll until it finishes.
for (const userPrompt of [
  ...
  let current = await albus.sessions.getSession({ id: session });
  while (current.session.state === "RUNNING") {
    await new Promise((resolve) => setTimeout(resolve, 2000));
    current = await albus.sessions.getSession({ id: session });
  }

  console.log(current.messages.at(-1)?.content);
}
```
