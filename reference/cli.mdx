...
`sessions run` prints the API response plus one key the CLI adds,
`idempotency_key` — the effective key of the invocation, which you resend to
re-attach to a run. See [Run a session](/guides/run-a-session).
